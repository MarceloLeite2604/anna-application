...
public class BroadcastReceiver extends android.content.BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (BluetoothAdapter.ACTION_DISCOVERY_STARTED.equals(action)) {
            Log.d(LOG_TAG, "onReceive, 25: Starting discovery.");
            pairer.discoverDevicesStarted();
        } else if (BluetoothAdapter.ACTION_DISCOVERY_FINISHED.equals(action)) {
            Log.d(LOG_TAG, "onReceive, 29: Finishing discovery.");
            pairer.discoverDevicesFinished();
        } else if (BluetoothDevice.ACTION_FOUND.equals(action)) {
            BluetoothDevice bluetoothDevice = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
            Log.d(LOG_TAG, "onReceive, 33: Device \"" + bluetoothDevice.getName() + "\" found.");
            ...
            if (previousState == BluetoothDevice.BOND_NONE && state == BluetoothDevice.BOND_BONDING) {
                Log.d(LOG_TAG, "onReceive, 39: Pairing started.");
                pairer.pairingStarted();
            } else if (previousState == BluetoothDevice.BOND_BONDING) {
                Log.d(LOG_TAG, "onReceive, 42: Pairing finished.");
