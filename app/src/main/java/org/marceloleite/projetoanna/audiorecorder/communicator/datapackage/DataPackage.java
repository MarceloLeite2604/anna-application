package org.marceloleite.projetoanna.audiorecorder.communicator.datapackage;
...
import org.marceloleite.projetoanna.audiorecorder.communicator.datapackage.content.ErrorContent;
...
import org.marceloleite.projetoanna.audiorecorder.communicator.datapackage.content.FileTrailerContent;
...
public class DataPackage {
     ...
     */
    public static final int PACKAGE_TRAILER_BYTE_ARRAY_SIZE = 4;
    ...
    public static int findPackageTrailerPosition(byte[] bytes) {
        if (bytes == null) {
            return -1;
        }

        if (bytes.length < PACKAGE_TRAILER_BYTE_ARRAY_SIZE) {
            return -1;
        }

        boolean searchConcluded = false;
        int searchPosition = 0;
        int packageTrailerPosition = 0;
        byte[] bytesChecked = new byte[PACKAGE_TRAILER_BYTE_ARRAY_SIZE];

        while (!searchConcluded) {
            if (searchPosition + PACKAGE_TRAILER_BYTE_ARRAY_SIZE > bytes.length) {
                packageTrailerPosition = -1;
                searchConcluded = true;
            } else {
                System.arraycopy(bytes, searchPosition, bytesChecked, 0, PACKAGE_TRAILER_BYTE_ARRAY_SIZE);
                if (checkByteIsPackageTrailer(bytesChecked)) {
                    packageTrailerPosition = searchPosition;
                    searchConcluded = true;
                } else {
                    searchPosition++;
                }
        ...
        return packageTrailerPosition;
    }

    private static boolean checkByteIsPackageTrailer(byte[] bytes) {
        int bytesInteger = ByteBuffer.wrap(bytes).getInt();
        return (bytesInteger == PACKAGE_TRAILER);
    }
}
